...
def html_to_word_ekipman_tablosu_table(html_content, doc):
    """Ekipman Tablosu HTML tablosunu dxa/tblGrid seviyesinde tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 2  # Tablo tam 2 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # 2 Sütun Orantılı Genişlikleri (Toplam 6.5 inç / 9360 dxa A4 Net Yazdırma Alanı)
    col_widths_inches = [2.50, 4.00]

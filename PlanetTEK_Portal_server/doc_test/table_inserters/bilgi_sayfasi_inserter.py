...
def html_to_word_bilgi_sayfasi_table(html_content, doc):
    """Bilgi Sayfası HTML tablosunu dxa/tblGrid seviyesinde tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 3  # Tablo tam 3 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # 3 Sütun Orantılı Genişlikleri (Toplam 6.5 inç / 9360 dxa A4 Net Yazdırma Alanı)
    col_widths_inches = [1.20, 2.80, 2.50]

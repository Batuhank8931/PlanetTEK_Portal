...
def html_to_word_enerji_karsilastirma_table(html_content, doc):
    """Enerji Karşılaştırma HTML tablosunu dxa/tblGrid seviyesinde tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 8  # Tablo 8 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # Sütun genişlikleri (Inches cinsinden) - Toplam: 6.5 inç (9360 dxa - Tam A4 Sayfa Genişliği)
    col_widths_inches = [1.15, 1.00, 0.55, 1.45, 0.85, 0.55, 0.45, 0.60]
    ...
    # 1. Tablo Genişliğini dxa (Sabit) Olarak Ayarla ve Autofit Kapat
    ...
    # 2. <w:tblGrid> Yapısını XML Seviyesinde Doldur (LibreOffice Sıkışmasını Engeller)
    ...
    for w_dxa in col_widths_dxa:
        ...
        tblGrid.append(gc)
    tblElem.insert(1, tblGrid)  # tblPr'den hemen sonra ekle

    # 3. Hücre ve Hücre Birleştirme İşlemleri
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            # Birleştirilmiş hücrenin toplam dxa genişliğini hesapla
            ...
            # Stil ve İçerik Biçimlendirmesi

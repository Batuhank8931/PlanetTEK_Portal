...
def get_parametre_row_count(html_path):
    """parametre.html dosyasındaki tr (satır) sayısını sayar."""
...
def clear_after_parameters():
    """
    ##clear_after_parametre## etiketini bulur ve eklenen satır sayısı kadar
    boş paragrafı (Enter) silerek altındaki içeriği yukarı çeker.
    """
    # clear_functions -> table_inserters -> doc_test
    ...
    # 1. Eklenen Satır Sayısını Al
    row_count = get_parametre_row_count(parametre_html_path)
    print(f"📊 Parametre tablosundaki satır sayısı: {row_count}")
    ...
    placeholder = "##clear_after_parametre##"
    ...
    # Etiketten önceki ve sonraki boş paragrafları tara
    # Önce yukarıya doğru boş paragrafları topla
    ...
    # Eğer yukarıdaki boşluklar yetmediyse aşağıya doğru boşlukları topla

...
def html_to_word_amortisman_table(html_content, doc):
    """Amortisman HTML tablosunu dxa/tblGrid seviyesinde tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 7  # Tablo tam 7 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # 7 Sütun Orantılı Genişlikleri (Toplam ~6.5 inç / 9360 dxa A4 Net Yazdırma Alanı)
    col_widths_inches = [1.50, 0.70, 0.75, 1.10, 0.75, 0.85, 0.85]

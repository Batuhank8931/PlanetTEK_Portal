...
def html_to_word_on_yillik_maliyet_table(html_content, doc):
    """HTML tablosunu dxa/tblGrid seviyesinde tam genişlikte Word belgesine dönüştürür."""
    ...
    num_rows = len(rows)
    num_cols = 5  # Tablo tam 5 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # 5 Sütun Orantılı Genişlikleri (Toplam ~6.5 inç / 9360 dxa A4 Net Yazdırma Alanı)
    col_widths_inches = [1.50, 1.45, 1.15, 1.15, 1.25]

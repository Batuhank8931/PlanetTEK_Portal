...
def convert_xlsx_to_html(excel_path, output_html_path):
    """OPEX Excel dosyasını alır ve birebir HTML'e çevirir."""
...
def process_opex_html():
    """Yalnızca opex.xlsx dosyasını hedef alıp opex.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("opex.xlsx"):
        excel_path = os.path.abspath("opex.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(base_dir, "tables", "htmls", "opex.html")
    else:
        excel_path = os.path.join(base_dir, "..", "tables", "xlsx1", "opex.xlsx")
        output_html_path = os.path.join(base_dir, "..", "tables", "htmls", "opex.html")
        os.makedirs(os.path.dirname(output_html_path), exist_ok=True)

    if not os.path.exists(excel_path):
        excel_path = "opex.xlsx"
        output_html_path = "opex.html"
    ...
    try:
        ...
        print(
            f"  ✅ OPEX HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ OPEX HTML Dönüşüm Hatası: {e}")

...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Bilgi Sayfası Excel dosyasını alır ve birebir HTML'e çevirir."""
...
def process_bilgi_sayfasi_html():
    """Yalnızca bilgi_sayfasi.xlsx dosyasını hedef alıp bilgi_sayfasi.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("bilgi_sayfasi.xlsx"):
        excel_path = os.path.abspath("bilgi_sayfasi.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "bilgi_sayfasi.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "bilgi_sayfasi.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "bilgi_sayfasi.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "bilgi_sayfasi.xlsx"
        output_html_path = "bilgi_sayfasi.html"
    ...
    try:
        ...
        print(
            f"  ✅ Bilgi Sayfası HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Bilgi Sayfası HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_bilgi_sayfasi_html()

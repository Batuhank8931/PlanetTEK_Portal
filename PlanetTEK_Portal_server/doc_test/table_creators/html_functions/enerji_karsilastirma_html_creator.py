...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Enerji Karşılaştırma Tablosu Excel dosyasını alır ve birebir HTML'e çevirir."""
    ...
    for r_idx in range(1, ws_data.max_row + 1):
        ...
        for c_idx in range(1, ws_data.max_column + 1):
            ...
            if raw_val is not None:
                if cell.font and cell.font.name == "Wingdings" and str(raw_val) == "ü":
                    val = "&#10003;"
                elif cell.number_format and (
...
def process_enerji_karsilastirma_html():
    """Yalnızca enerji_karsilastirma.xlsx dosyasını hedef alıp enerji_karsilastirma.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("enerji_karsilastirma.xlsx"):
        excel_path = os.path.abspath("enerji_karsilastirma.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "enerji_karsilastirma.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "enerji_karsilastirma.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "enerji_karsilastirma.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "enerji_karsilastirma.xlsx"
        output_html_path = "enerji_karsilastirma.html"
    ...
    try:
        ...
        print(
            f"  ✅ Enerji Karşılaştırma HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Enerji Karşılaştırma HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_enerji_karsilastirma_html()

...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Amortisman Excel dosyasını alır ve boş satırları kırparak birebir HTML'e çevirir."""
    ...
    # 1. Adım: Gerçek verinin veya biçimlendirmenin bittiği son satırı bul (Boşlukları kırp)
    last_row = 0
    for r_idx in range(ws_data.max_row, 0, -1):
        has_content = any(
            ws_data.cell(row=r_idx, column=c).value is not None
            for c in range(1, ws_data.max_column + 1)
        )
        has_styling = any(
            ws_data.cell(row=r_idx, column=c).fill.fill_type
            or ws_data.cell(row=r_idx, column=c).border.top.style
            or ws_data.cell(row=r_idx, column=c).border.bottom.style
            for c in range(1, ws_data.max_column + 1)
        )
        if has_content or has_styling:
            last_row = r_idx
            break
    ...
    # Sadece gerçek içeriğin olduğu satıra kadar dön (Boş alt satırlar kırpılır)
    for r_idx in range(1, last_row + 1):
        ...
        for c_idx in range(1, ws_data.max_column + 1):
            ...
            cell_formula = ws_formula.cell(row=r_idx, column=c_idx)
...
def process_amortisman_html():
    """Yalnızca amortisman.xlsx dosyasını hedef alıp amortisman.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("amortisman.xlsx"):
        excel_path = os.path.abspath("amortisman.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(base_dir, "tables", "htmls", "amortisman.html")
    else:
        excel_path = os.path.join(base_dir, "..", "tables", "xlsx1", "amortisman.xlsx")
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "amortisman.html"
        )
        os.makedirs(os.path.dirname(output_html_path), exist_ok=True)

    if not os.path.exists(excel_path):
        excel_path = "amortisman.xlsx"
        output_html_path = "amortisman.html"
    ...
    try:
        ...
        print(
            f"  ✅ Amortisman HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Amortisman HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_amortisman_html()

...
def set_cell_margins(cell, top=30, bottom=30, left=40, right=40):
    """Hücre içi padding ayarları (Ekipman tablosu için optimize edildi)."""
...
def html_to_word_ekipman_tablosu_table(html_content, doc):
    """Ekipman Tablosu HTML tablosunu 2 sütunlu yapısıyla ve tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 2  # Tablo tam 2 sütunlu[cite: 10]
    ...
    # 2 Sütun için orantılı genişlikler (Toplam ~6.5 inç A4 net alan)[cite: 10]
    col_widths = [
        Inches(2.50),  # Özellik Adı (Sol Sütun)[cite: 10]
        Inches(4.00),  # Değer / Detay (Sağ Sütun)[cite: 10]
    ]
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            size_val = float(size_match.group()) if size_match else 10.0
...
def process_ekipman_tablosu_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "ekipman_tablosu.html")
    ...
    if target_p:
        print(f"  🔍 '{placeholder}' etiketi bulundu. Ekipman Tablosu ekleniyor...")

        word_table = html_to_word_ekipman_tablosu_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Ekipman Tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_ekipman_tablosu_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Ekipman Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_ekipman_tablosu_and_insert()

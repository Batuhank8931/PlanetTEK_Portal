...
def set_cell_margins(cell, top=40, bottom=40, left=50, right=50):
    """Hücre içi padding ayarları (Sarf malzeme tablosu için optimize edildi)."""
...
def html_to_word_sarf_table(html_content, doc):
    """Sarf Malzeme HTML tablosunu 5 sütunlu yapısıyla ve ince kenarlıklarıyla Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 5  # Sarf malzeme tablosu tam 5 sütunlu
    ...
    col_widths = [
        Inches(2.15),  # 323px -> Açıklama / Description
        Inches(1.10),  # 170px -> Total Amount
        Inches(1.35),  # 212px -> Consumption
        Inches(0.95),  # 153px -> Unit Price
        Inches(1.20),  # 187px -> Total Price
    ]
...
def process_sarf_malzeme_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "sarf_malzeme.html")
    ...
    if target_p:
        print(
            f"  🔍 '{placeholder}' etiketi bulundu. Sarf Malzeme tablosu ekleniyor..."
        )

        word_table = html_to_word_sarf_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Sarf Malzeme tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_sarf_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Sarf Malzeme Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_sarf_malzeme_and_insert()

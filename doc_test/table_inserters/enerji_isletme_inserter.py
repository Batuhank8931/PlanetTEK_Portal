...
def set_cell_margins(cell, top=40, bottom=40, left=50, right=50):
    """Hücre içi padding ayarları (Çok sütunlu enerji tablosu için optimize edildi)."""
...
def html_to_word_enerji_table(html_content, doc):
    """Enerji İşletme HTML tablosunu 8 sütunlu yapısıyla ve ince kenarlıklarıyla Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 8  # Enerji tablosu tam 8 sütunlu

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # HTML <colgroup> içerisindeki pikselleri Ingress/Inches değerlerine dönüştürme (Toplam ~6.5 inç A4 Genişliği)
    col_widths = [
        Inches(1.70),  # 323px -> Açıklama
        Inches(0.60),  # 102px -> Unit
        Inches(0.40),  # 68px  -> Unit 2
        Inches(0.75),  # 119px -> kW
        Inches(0.75),  # 119px -> kW
        Inches(0.75),  # 119px -> %
        Inches(0.75),  # 119px -> hour
        Inches(0.80),  # 153px -> kW.hour/day
    ]
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            # Yazı rengi desteği (Örn: Kırmızı uyarı yazıları için)
...
def process_enerji_isletme_and_insert():
    ...
    html_file_path = os.path.join(
        tables_dir, "htmls", "enerji_isletme.html"
    )  # HTML yolunu ihtiyacınıza göre ayarlayabilirsiniz
    ...
    if not os.path.exists(html_file_path):
        # Alternatif yoldan aramayı dene
        alt_html_path = os.path.join(script_dir, "enerji_isletme.html")
        if os.path.exists(alt_html_path):
            html_file_path = alt_html_path
        else:
            print(f"Hata: 'enerji_isletme.html' dosyası bulunamadı!")
            return

    print(f"📄 Word belgesi yükleniyor: {os.path.basename(target_docx_path)}")
    doc = Document(target_docx_path)

    placeholder = "##enerji_isletme##"
    ...
    if target_p:
        print(
            f"  🔍 '{placeholder}' etiketi bulundu. Enerji İşletme tablosu ekleniyor..."
        )

        word_table = html_to_word_enerji_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Enerji İşletme tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_enerji_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Enerji İşletme Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_enerji_isletme_and_insert()

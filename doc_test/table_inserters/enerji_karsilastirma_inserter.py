...
def set_cell_background(cell, fill_hex):
    """Hücre arka plan rengini ayarlar[cite: 4]."""
    if not fill_hex:
        return
...
def set_cell_margins(cell, top=30, bottom=30, left=40, right=40):
    """Hücre içi padding ayarları (Enerji karşılaştırma tablosu için optimize edildi)."""
...
def parse_inline_styles(style_str):
    """HTML inline CSS stringini dictionary olarak dönüştürür[cite: 4]."""
    styles = {}
    if not style_str:
        return styles
...
def parse_border_css(border_css_str):
    """CSS border metnini 1/4 pt (sz: 2) standart Word border objesine dönüştürür[cite: 4]."""
...
def html_to_word_enerji_karsilastirma_table(html_content, doc):
    """Enerji Karşılaştırma HTML tablosunu 8 sütunlu yapısıyla ve ince kenarlıklarıyla Word'e aktarır[cite: 4]."""
    ...
    num_rows = len(rows)
    num_cols = 8  # Tablo tam 8 sütunlu[cite: 4]

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # HTML <colgroup> içerisindeki pikselleri Inches değerlerine dönüştürme (Toplam ~6.5 inç A4 Genişliği)[cite: 4]
    # HTML <colgroup> piksellerini oranlayarak toplamı tam 6.5 inç (A4 net yazdırılabilir alan) yapıyoruz
    col_widths = [
        Inches(1.15),  # 1. Sütun
        Inches(1.00),  # 2. Sütun
        Inches(0.55),  # 3. Sütun
        ...
        Inches(0.45),  # 7. Sütun
        Inches(0.60),  # 8. Sütun
    ]  # Toplam: ~6.5 inç (Sayfanın sol ve sağ boşluklarını tamamen doldurur)

    tblPr = table._tbl.tblPr
    for old_w in tblPr.findall(qn("w:tblW")):
        tblPr.remove(old_w)

    # Tabloyu yüzde 100 genişliğe (sayfa kenarlarına tam oturacak şekilde) sabitleme
    tblW = OxmlElement("w:tblW")
    tblW.set(
        qn("w:w"), "5000"
    )  # Word'de %100 genişlik karşılığı 5000'dir (dxa yerine pct)
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            # Font boyutu (Eğer 26pt gibi özel büyük fontlar varsa onları destekler)[cite: 4]
            ...
            # Yazı rengi desteği (Örn: Kırmızı vurgular için)[cite: 4]
            ...
            # Kenarlıklar (Tüm borderlar 1/4 pt yani sz="2" olarak sabitleniyor)[cite: 4]
            border_opts = {}
    ...
    # Sütun Genişliklerinin Tabloya Uygulanması
...
def process_enerji_karsilastirma_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "enerji_karsilastirma.html")
    ...
    if target_p:
        print(
            f"  🔍 '{placeholder}' etiketi bulundu. Enerji Karşılaştırma tablosu ekleniyor..."
        )

        word_table = html_to_word_enerji_karsilastirma_table(html_content, doc)
        ...
        if word_table:
            ...
            print(
                "  ✅ Enerji Karşılaştırma tablosu yerleştirildi ve etiket kaldırıldı."
            )
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_enerji_karsilastirma_table(html_content, doc)

    doc.save(output_path)
    print(
        f"\n🎉 Enerji Karşılaştırma Tablosu Aktarımı Tamamlandı! Dosya: {output_path}"
    )


if __name__ == "__main__":
    process_enerji_karsilastirma_and_insert()

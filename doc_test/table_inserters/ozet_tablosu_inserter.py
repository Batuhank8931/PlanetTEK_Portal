...
def set_cell_margins(cell, top=30, bottom=30, left=40, right=40):
    """Hücre içi padding ayarları (Özet tablosu için optimize edildi)."""
...
def html_to_word_ozet_tablosu_table(html_content, doc):
    """Özet Tablosu HTML tablosunu 6 sütunlu yapısıyla ve tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 6  # Tablo tam 6 sütunlu[cite: 9]
    ...
    # 6 Sütun için orantılı genişlikler (Toplam ~6.5 inç A4 net alan)[cite: 9]
    col_widths = [
        Inches(0.35),  # 35px
        Inches(0.70),  # 67px
        Inches(1.00),  # 104px
...
def process_ozet_tablosu_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "ozet_tablosu.html")
    ...
    if target_p:
        print(f"  🔍 '{placeholder}' etiketi bulundu. Özet Tablosu ekleniyor...")

        word_table = html_to_word_ozet_tablosu_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Özet Tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_ozet_tablosu_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Özet Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_ozet_tablosu_and_insert()

...
def set_cell_background(cell, fill_hex):
    """Hücre arka plan rengini ayarlar[cite: 8]."""
    if not fill_hex:
        return
...
def set_cell_margins(cell, top=30, bottom=30, left=40, right=40):
    """Hücre içi padding ayarları (Bilgi sayfası tablosu için optimize edildi)."""
...
def parse_inline_styles(style_str):
    """HTML inline CSS stringini dictionary olarak dönüştürür[cite: 8]."""
    styles = {}
    if not style_str:
        return styles
...
def html_to_word_bilgi_sayfasi_table(html_content, doc):
    """Bilgi Sayfası HTML tablosunu 3 sütunlu yapısıyla ve tam genişlikte Word'e aktarır[cite: 8]."""
    ...
    num_rows = len(rows)
    num_cols = 3  # Tablo tam 3 sütunlu[cite: 8]

    table = doc.add_table(rows=num_rows, cols=num_cols)
    table.autofit = False
    table.allow_autofit = False

    # Sayfa kenarlarına tam yaslanması için %100 genişlik (pct) ayarı
    tblPr = table._tbl.tblPr
    for old_w in tblPr.findall(qn("w:tblW")):
        tblPr.remove(old_w)
    tblW = OxmlElement("w:tblW")
    tblW.set(qn("w:w"), "5000")  # Word'de %100 genişlik karşılığı
    tblW.set(qn("w:type"), "pct")
    tblPr.append(tblW)

    # 3 Sütun için orantılı genişlikler (Toplam ~6.5 inç A4 net alan)[cite: 8]
    col_widths = [
        Inches(1.20),  # 1. Sütun (Boşluk/Etiket)
        Inches(2.80),  # 2. Sütun (Açıklama/Başlık)
        Inches(2.50),  # 3. Sütun (Değer/Detay)
    ]
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            # Font boyutu (14pt ve 12pt gibi büyük başlık fontlarını destekler)[cite: 8]
            ...
            # Kenarlıklar (Tüm borderlar 1/4 pt yani sz="2" olarak sabitleniyor)
            border_opts = {}
...
def process_bilgi_sayfasi_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "bilgi_sayfasi.html")
    ...
    if target_p:
        print(
            f"  🔍 '{placeholder}' etiketi bulundu. Bilgi Sayfası tablosu ekleniyor..."
        )

        word_table = html_to_word_bilgi_sayfasi_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Bilgi Sayfası tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_bilgi_sayfasi_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Bilgi Sayfası Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_bilgi_sayfasi_and_insert()

...
def set_cell_margins(cell, top=30, bottom=30, left=40, right=40):
    """Hücre içi padding ayarları (Amortisman tablosu için optimize edildi)."""
...
def html_to_word_amortisman_table(html_content, doc):
    """Amortisman HTML tablosunu 7 sütunlu yapısıyla ve tam genişlikte Word'e aktarır."""
    ...
    num_rows = len(rows)
    num_cols = 7  # Tablo tam 7 sütunlu[cite: 7]
    ...
    # 7 Sütun için orantılı genişlikler (Toplam ~6.5 inç A4 net alan)[cite: 7]
    col_widths = [
        Inches(1.50),  # 1. Sütun
        Inches(0.70),  # 2. Sütun
        Inches(0.75),  # 3. Sütun
    ...
    for r_idx, tr in enumerate(rows):
        ...
        for td in cells:
            ...
            # Yazı rengi desteği (Örn: Kırmızı vurgular için)[cite: 7]
...
def process_amortisman_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "amortisman.html")
    ...
    if not os.path.exists(html_file_path):
        alt_html_path = os.path.join(script_dir, "amortisman.html")
        if os.path.exists(alt_html_path):
            html_file_path = alt_html_path
        else:
            print(f"Hata: 'amortisman.html' dosyası bulunamadı!")
            return
    ...
    if target_p:
        print(f"  🔍 '{placeholder}' etiketi bulundu. Amortisman tablosu ekleniyor...")

        word_table = html_to_word_amortisman_table(html_content, doc)
        ...
        if word_table:
            ...
            print("  ✅ Amortisman tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f"  ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_amortisman_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 Amortisman Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_amortisman_and_insert()

...
def html_to_word_on_yillik_maliyet_table(html_content, doc):
    """HTML tablosunu Word belgesine tam genişlikte dönüştürür."""
    ...
    num_rows = len(rows)
    num_cols = 5  # HTML 5 sütunlu olduğu için kesinlikle 5 olmalı
    ...
    tblW.set(qn("w:w"), "5000")  # Word XML: %100 genişlik
    tblW.set(qn("w:type"), "pct")
    tblPr.append(tblW)

    # 5 Sütun Orantılı Genişlikleri (Toplam ~6.5 inç / A4 Yazdırma Alanı)
    col_widths = [
        Inches(1.50),  # Sistem Adı
        Inches(1.45),  # İlk Yatırım Maliyeti
        Inches(1.15),  # Yıllık Enerji
        Inches(1.15),  # Yıllık Operatör
        Inches(1.25),  # Yıllık Bakım
    ]
    ...
    # Genişliklerin hücre seviyesinde doğru oturtulması
...
def process_on_yillik_maliyet_and_insert():
    ...
    html_file_path = os.path.join(tables_dir, "htmls", "on_yillik_maliyet.html")
    ...
    if target_p:
        print(
            f" 🔍 '{placeholder}' etiketi bulundu. 10 Yıllık Maliyet tablosu ekleniyor..."
        )

        word_table = html_to_word_on_yillik_maliyet_table(html_content, doc)
        ...
        if word_table:
            ...
            print(" ✅ 10 Yıllık Maliyet tablosu yerleştirildi ve etiket kaldırıldı.")
    else:
        print(
            f" ⚠️ Belge içinde '{placeholder}' etiketi bulunamadı, tablo sona ekleniyor..."
        )
        html_to_word_on_yillik_maliyet_table(html_content, doc)

    doc.save(output_path)
    print(f"\n🎉 10 Yıllık Maliyet Tablosu Aktarımı Tamamlandı! Dosya: {output_path}")


if __name__ == "__main__":
    process_on_yillik_maliyet_and_insert()

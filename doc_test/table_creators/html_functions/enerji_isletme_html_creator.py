...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Enerji İşletme Excel dosyasını alır ve birebir HTML'e çevirir."""
...
def process_enerji_isletme_html():
    """Yalnızca enerji_isletme.xlsx dosyasını hedef alıp enerji_isletme.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    xlsx_dir = os.path.join(base_dir, "..", "tables", "xlsx")
    html_dir = os.path.join(base_dir, "..", "tables", "htmls")

    if os.path.exists("enerji_isletme.xlsx"):
        excel_path = os.path.abspath("enerji_isletme.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "enerji_isletme.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "enerji_isletme.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "enerji_isletme.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "enerji_isletme.xlsx"
        output_html_path = "enerji_isletme.html"
    ...
    try:
        ...
        print(
            f"  ✅ Enerji İşletme HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Enerji İşletme HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_enerji_isletme_html()

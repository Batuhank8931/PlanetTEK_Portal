...
def convert_xlsx_to_html(excel_path, output_html_path):
    ...
    # GEREKSİZ ALT BOŞ SATIRLARI FİLTRELEME:
    last_real_row = ws_data.max_row
    while last_real_row > 0:
        row_cells = [
            ws_data.cell(row=last_real_row, column=c).value
            for c in range(1, ws_data.max_column + 1)
        ]
        if any(val is not None and str(val).strip() != "" for val in row_cells):
            break
        last_real_row -= 1
    ...
    for r_idx in range(1, last_real_row + 1):
        ...
        for c_idx in range(1, ws_data.max_column + 1):
            ...
            if raw_val is not None:
                ...
                # Wingdings fontunda tik sembolü olan 'ü' karakterini '✔' ile değiştir
                if val.strip() == "ü":
                    val = "✔"
            else:
                val = ""
...
def process_ozet_tablosu_html():
    """Yalnızca ozet_tablosu.xlsx dosyasını hedef alıp ozet_tablosu.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("ozet_tablosu.xlsx"):
        excel_path = os.path.abspath("ozet_tablosu.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "ozet_tablosu.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "ozet_tablosu.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "ozet_tablosu.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "ozet_tablosu.xlsx"
        output_html_path = "ozet_tablosu.html"
    ...
    try:
        ...
        print(
            f"  ✅ Özet Tablosu HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Özet Tablosu HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_ozet_tablosu_html()

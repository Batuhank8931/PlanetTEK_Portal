...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Ekipman Tablosu Excel dosyasını alır ve birebir HTML'e çevirir."""
...
def process_ekipman_tablosu_html():
    """Yalnızca ekipman_tablosu.xlsx dosyasını hedef alıp ekipman_tablosu.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("ekipman_tablosu.xlsx"):
        excel_path = os.path.abspath("ekipman_tablosu.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "ekipman_tablosu.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "ekipman_tablosu.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "ekipman_tablosu.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "ekipman_tablosu.xlsx"
        output_html_path = "ekipman_tablosu.html"
    ...
    try:
        ...
        print(
            f"  ✅ Ekipman Tablosu HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Ekipman Tablosu HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_ekipman_tablosu_html()

...
def convert_xlsx_to_html(excel_path, output_html_path):
    """Karbon Ayakizi Excel dosyasını alır ve birebir HTML'e çevirir."""
...
def process_karbon_ayakizi_html():
    """Yalnızca karbon_ayakizi.xlsx dosyasını hedef alıp karbon_ayakizi.html üretir."""
    base_dir = os.path.dirname(os.path.abspath(__file__))

    if os.path.exists("karbon_ayakizi.xlsx"):
        excel_path = os.path.abspath("karbon_ayakizi.xlsx")
        os.makedirs(os.path.join(base_dir, "tables", "htmls"), exist_ok=True)
        output_html_path = os.path.join(
            base_dir, "tables", "htmls", "karbon_ayakizi.html"
        )
    else:
        excel_path = os.path.join(
            base_dir, "..", "tables", "xlsx1", "karbon_ayakizi.xlsx"
        )
        output_html_path = os.path.join(
            base_dir, "..", "tables", "htmls", "karbon_ayakizi.html"
        )
    ...
    if not os.path.exists(excel_path):
        excel_path = "karbon_ayakizi.xlsx"
        output_html_path = "karbon_ayakizi.html"
    ...
    try:
        ...
        print(
            f"  ✅ Karbon Ayakizi HTML Dönüşümü Başarılı: {os.path.basename(excel_path)} -> {os.path.basename(output_html_path)}"
        )
    except Exception as e:
        print(f"  ❌ Karbon Ayakizi HTML Dönüşüm Hatası: {e}")


if __name__ == "__main__":
    process_karbon_ayakizi_html()

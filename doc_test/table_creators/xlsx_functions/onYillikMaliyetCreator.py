...
# --- YENİ JSON YAPISINA GÖRE PARSE ETME ---
customer_info = form_data.get("customerInfo", {})
data_obj = form_data.get("tables", {}).get("onyillikmaliyettablosu", {})

# DÜZELTME: Verilerin altındaki "data" sözlüğüne ve tablo içi alanlara doğru erişim
inner_data = data_obj.get("data", {})
selected_system = data_obj.get(
    "selectedSystem", form_data.get("selectedSystem", "mbbr")
)
total_savings_10y = data_obj.get("totalSavings10Y", 0)
inflation_rate = inner_data.get("inflationRate", 0)
...
# PlanetDISK Verisi (inner_data altından okunuyor)
planet_raw = inner_data.get("planet", {})
if planet_raw:
    rows_data.append(
        {
            "system": "PlanetDISK®",
            "capex": planet_raw.get("capex", 0),
            "energy": planet_raw.get("energy", 0),
            "operator": planet_raw.get("operator", 0),
            "maint": planet_raw.get("maintenance", 0),
        }
    )

# Karşılaştırılan Sistem Verisi (mbbr veya aktif_camur - inner_data altından okunuyor)
target_sys_raw = inner_data.get(selected_system, {})
if target_sys_raw:
    system_label = "MBBR" if is_mbbr else "Aktif Çamur"
    rows_data.append(
        {
            "system": system_label,
            "capex": target_sys_raw.get("capex", 0),
            "energy": target_sys_raw.get("energy", 0),
            "operator": target_sys_raw.get("operator", 0),
            "maint": target_sys_raw.get("maintenance", 0),
        }
    )

# --- ALT VURGU METNİ (Footer Gain) ---
if is_foreign:
    footer_gain_text = f"10 Years Total Savings with PlanetDISK®: {total_savings_10y:,.2f} {CURRENCY_SYMBOL}"
else:
    footer_gain_text = f"PlanetDISK® ile 10 Yıllık Toplam Kazanç: {total_savings_10y:,.2f} {CURRENCY_SYMBOL}"
...
# ==========================================
# 5. SATIR 3-4: TABLO VERİ SATIRLARI
# ==========================================
current_row = 3
...
for row_item in rows_data:
    ...
    capex_val = row_item.get("capex", 0)
    energy_val = row_item.get("energy", 0)
    op_val = row_item.get("operator", 0)
    maint_val = row_item.get("maint", 0)

...
function IleriAritmaPumpSelections({ onFlowChange }) {
  const CALC_HOURS = 24;
  ...
  useEffect(() => {
    const fetchPumps = async () => {
      try {
        ...
        console.error("Geri devir pompası verileri yüklenemedi:", error);
      }
    };
    fetchPumps();
  }, []);
  ...
  // 🚀 PARENT'A AKIŞ BİLDİRİMİ: Debinin değiştiği her an Parent Component güncellenir
  useEffect(() => {
    if (onFlowChange) {
      onFlowChange(activeHourlyFlow);
    }
  }, [activeHourlyFlow, onFlowChange]);

...
function EnerjiIsletmeTablosu() {
    // Üst kısımdaki genel parametreler (Hidrolik Yük ve Enerji Fiyatı)
    const [params, setParams] = useState({
        hydraulicLoad: 2.92, // m³/saat
        energyPrice: 13,     // € cent
    });

    // Hiyerarşik veri seti (Başlıklar ve Alt Kalemler)
    ...
    // --- HESAPLAMA MOTORU ---
    // Tek bir satırın günlük elektrik tüketimini (kWh/gün) hesaplar
    ...
    const getRowBg = (row) => {
        ...
        // Miktarı 0 olan satırlar için görseldeki gibi hafif kırmızımtırak bir renk efekti (Dark mode uyumlu)
    ...
    return (
      ...
      `}</style>
            ...
            {/* ÜST KONTROL VE PARAMETRE PANELİ */}
            <div className="d-flex justify-content-between align-items-end mb-2">
                ...
                >
                    <span style={{ fontSize: "12px" }}>↶</span> Geri Al ({history.length})
                </button>
            </div>

            {/* BÜYÜK DİNAMİK TABLO (7 KOLON) */}
            <div className="d-flex flex-column rounded-3" style={{ border: "1px solid #334155", height: "auto", overflowX: "hidden" }}>
                ...
                {rows.map((row, index) => {
                    ...
                    // Görseldeki gibi adet veya toplam güç sıfırsa kırmızı metin/hücre efekti
                    const isZero = q === 0;
                    const numColor = isZero ? "#ef4444" : "white"; // Kırmızımsı text
                    ...
                    return (
                        <div key={row.id} className="d-flex align-items-stretch energy-row" style={{ backgroundColor: getRowBg(row) }}>
                            ...
                            {/* 7. Elektrik Tüketimi (kWh/gün) - Hesaplanır */}
            ...
            {/* ALT TOPLAM VE ÖZET PANELİ (Görseldeki Sonuç Tablosu) */}

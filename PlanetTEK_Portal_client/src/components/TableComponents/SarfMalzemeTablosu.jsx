...
function SarfMalzemeTablosu() {
  // Görseldeki tüm sarf malzeme ve bakım kalemlerinin Türkçe veri seti
  ...
  const calculateRowTotal = (row) => {
    ...
    // Not: FeCl3 ve Polielektrolit gibi bazı kalemlerde tüketim ton/yıl direkt toplam da olabilir.
    // Excel formül akışına sadık kalmak adına doğrusal çarpan uygulanmıştır.
    return qty === 0 ? 0 : qty * consumption * unitPrice;
  };

  // Tüm satırların yıllık toplam tutarlarının toplamı (Genel Toplam)
  ...
  const getRowBg = (row) => {
    ...
    if (!row.isHeader && !row.isSubHeader && (parseFloat(row.qty) === 0)) return "#2a1515"; // Miktarı 0 olanlar için hafif kırmızı dolgu
    return "#151f32"; 
  };
  ...
  return (
    <div className="d-flex flex-column gap-3 w-100">
      ...
      {/* ANA TABLO GRİD YAPISI (6 KOLONLU) */}
